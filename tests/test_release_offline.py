...
def test_version_is_synchronized():
    assert att.__version__ == "1.2.4"
    assert 'version="1.2.4"' in (ROOT / "setup.py").read_text(encoding="utf-8")
    assert "current_version = 1.2.4" in (ROOT / "setup.cfg").read_text(encoding="utf-8")
...
def test_pdf_guide_generator_tracks_release_and_rtl_layout():
    ...
    assert 'VERSION = "1.2.4"' in generator
    assert "AlgoTik_TSE_Guide_v1.2.4_preview.pdf" in generator
...
def test_markdown_is_single_authoritative_reference_and_covers_public_exports():
    ...
    for badge in (
        "img.shields.io/badge/pypi-v1.2.4-blue.svg",
