...
def test_version_is_synchronized():
    assert att.__version__ == "1.3.0"
    assert 'version="1.3.0"' in (ROOT / "setup.py").read_text(encoding="utf-8")
    assert "current_version = 1.3.0" in (ROOT / "setup.cfg").read_text(encoding="utf-8")
    history = (ROOT / "HISTORY.rst").read_text(encoding="utf-8")
    assert "1.3.0 (2026-09-12)" in history
...
def test_pdf_guide_generator_tracks_release_and_rtl_layout():
    ...
    assert 'VERSION = "1.3.0"' in generator
    assert "AlgoTik_TSE_Guide_v1.3.0_preview.pdf" in generator
...
def test_markdown_is_single_authoritative_reference_and_covers_public_exports():
    ...
    for badge in (
        "img.shields.io/badge/pypi-v1.3.0-blue.svg",
...
def test_markdown_has_exact_signature_and_contract_entry_for_every_callable():
    ...
    assert len(att.__all__) == 128
    assert len(callables) == 122
    missing_signatures = []
